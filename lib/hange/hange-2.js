"use strict";
/**
 * 根据输入的数组中每项的 before/after/first/last 规则，输出一个新排好序的数组或者链表
 * 题解：
 * 1. 这是一个典型的链表关系的数据结构
 * 2. 先使用原始数组构造一个链表，因为后面步骤需要调整链表元素的位置，因此这个链表不仅有 next 属性，还有个 prev 属性，两个相邻的链表元素做双向关联
 * 3. 为了方便根据 id 找到列表元素，将元素都加入新数组 map 中
 * 4. 为了方便链表元素的移动（尤其是应用 before, after 规则），在链表的头部和尾部都增加一个空元素 => first -> 1 -> 2 -> 3 -> 5 -> 6 -> 7 -> 8 -> 9 -> last
 * 5. 遍历数组 map，根据 before/after/first/last 规则调整元素在链表中的位置，生成最终的链表
 * 6. 遍历第5步生成的链表，核对 before/after/first/last 的合法性
 */
var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m;
Object.defineProperty(exports, "__esModule", { value: true });
// 根据输入的数组中每项的 before/after/first/last 规则，输出一个新排好序的数组或者链表。要求，多解的情况可以只求一解，如果无解要求程序能检测出来。
var input = [
    { id: 1 },
    { id: 2, before: 1 },
    { id: 3, after: 1 },
    { id: 5, first: true },
    { id: 6, last: true },
    { id: 7, after: 8 },
    { id: 8, after: 8 },
    { id: 9 },
];
// 链表
var List = /** @class */ (function () {
    function List(val, next, prev) {
        this.val = val;
        this.next = next;
        this.prev = prev;
    }
    return List;
}());
// 连接两个元素
var concat = function (prev, next) {
    prev.next = next;
    next.prev = prev;
};
// 从链表中提取元素
var extract = function (list) {
    list.prev && (list.prev.next = list.next);
    list.next && (list.next.prev = list.prev);
};
// 将提取的元素移至 target 之前
var moveBefore = function (list, target) {
    extract(list);
    list.prev = target.prev;
    list.next = target;
    list.prev && (list.prev.next = list);
    list.next && (list.next.prev = list);
};
// 将提取的元素移至 target 之后
var moveAfter = function (list, target) {
    extract(list);
    list.prev = target;
    list.next = target.next;
    list.prev && (list.prev.next = list);
    list.next && (list.next.prev = list);
};
var first = new List();
var last = new List();
var temp = first;
var map = {};
// 使用初始顺序初始化链表 => first -> 1 -> 2 -> 3 -> 5 -> 6 -> 7 -> 8 -> 9 -> last
input.forEach(function (item) {
    concat(temp, (map[item.id] = new List(item)));
    temp.next && (temp = temp.next);
});
concat(temp, last);
// 遍历链表，调整元素顺序
input.forEach(function (item) {
    var list = map[item.id];
    if (item.before) {
        var target = map[item.before];
        extract(list);
        moveBefore(list, target);
    }
    else if (item.after) {
        var target = map[item.after];
        extract(list);
        moveAfter(list, target);
    }
    else if (item.first) {
        extract(list);
        moveAfter(list, first);
    }
    else if (item.last) {
        extract(list);
        moveBefore(list, last);
    }
});
var result = [];
var cur = first;
var count = 0;
var maxTimes = 1000;
while (cur.next && count <= maxTimes) {
    count++;
    cur = cur.next;
    // 链表有效性校验
    if ((_a = cur.val) === null || _a === void 0 ? void 0 : _a.first) {
        if (((_b = cur.prev) === null || _b === void 0 ? void 0 : _b.val) !== void 0) {
            console.error('Duplicated first item');
        }
    }
    else if ((_c = cur.val) === null || _c === void 0 ? void 0 : _c.last) {
        if (((_d = cur.next) === null || _d === void 0 ? void 0 : _d.val) !== void 0) {
            console.error('Duplicated last item');
        }
    }
    else if ((_e = cur.val) === null || _e === void 0 ? void 0 : _e.before) {
        if (((_g = (_f = cur.next) === null || _f === void 0 ? void 0 : _f.val) === null || _g === void 0 ? void 0 : _g.id) !== ((_h = cur.val) === null || _h === void 0 ? void 0 : _h.before)) {
            console.error('Error item', cur.val);
        }
    }
    else if ((_j = cur.val) === null || _j === void 0 ? void 0 : _j.after) {
        if (((_l = (_k = cur.prev) === null || _k === void 0 ? void 0 : _k.val) === null || _l === void 0 ? void 0 : _l.id) !== ((_m = cur.val) === null || _m === void 0 ? void 0 : _m.after)) {
            console.error('Error item', cur.val);
        }
    }
    if (cur.next) {
        result.push(cur.val);
    }
}
console.log('Result: ', result);
exports.default = {};
